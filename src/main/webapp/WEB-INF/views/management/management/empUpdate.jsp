...
<script type="text/javascript"
    src="//dapi.kakao.com/v2/maps/sdk.js?appkey=e08ba589b52c5c3e16fd367810537e20&libraries=services"></script>
...
<div class="body-wrapper">
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="/management/update?empId=${employeeVO.empId}" method="post" id="empRegisterForm"
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <div class="controls">
                            <input type="text" name="empId" id="empId" class="form-control" placeholder="자동 생성"
                                value="${employeeVO.empId}" disabled />
                        </div>
                    ...
                    <div class="mb-3 form-group col-12">
                        ...
                        <div class="card-body p-4 col-3">
                            ...
                            <div class="text-center">
                                <img src="${employeeVO.empImage}" alt="modernize-img" class="img-fluid rounded-circle"
                                    width="120" height="120" id="profileImg">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <div class="controls">
                            <select name="empDept" id="empDept" required class="form-select form-control">
                                <option value="인사부" <c:if test="${employeeVO.empDept eq '인사부'}">selected</c:if> >인사부
                                </option>
                                <option value="전산부" <c:if test="${employeeVO.empDept eq '전산부'}">selected</c:if> >전산부
                                </option>
                                <option value="인사부"
                                    <c:if test="${employeeVO.empDept eq '협력업체 관리부'}">selected</c:if> >협력업체 관리부
                                </option>
                                <option value="공항서비스 관리부"
                                    <c:if test="${employeeVO.empDept eq '공항서비스 관리부'}">selected</c:if> >공항서비스 관리부
                                </option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        ...
                        <div class="controls">
                            <select name="empPosition" id="empPosition" class="form-select form-control">
                                <option value="사원" <c:if test="${employeeVO.empPosition eq '사원'}">selected</c:if>>사원
                                </option>
                                <option value="주임" <c:if test="${employeeVO.empPosition eq '주임'}">selected</c:if>>주임
                                </option>
                                <option value="대리" <c:if test="${employeeVO.empPosition eq '대리'}">selected</c:if>>대리
                                </option>
                                <option value="과장" <c:if test="${employeeVO.empPosition eq '과장'}">selected</c:if>>과장
                                </option>
                                <option value="차장" <c:if test="${employeeVO.empPosition eq '차장'}">selected</c:if>>차장
                                </option>
                                <option value="부장" <c:if test="${employeeVO.empPosition eq '부장'}">selected</c:if>>부장
                                </option>
                                <option value="이사" <c:if test="${employeeVO.empPosition eq '이사'}">selected</c:if>>이사
                                </option>
                                <option value="상무" <c:if test="${employeeVO.empPosition eq '상무'}">selected</c:if>>상무
                                </option>
                                <option value="전무" <c:if test="${employeeVO.empPosition eq '전무'}">selected</c:if>>전무
                                </option>
                                <option value="부사장" <c:if test="${employeeVO.empPosition eq '부사장'}">selected</c:if>>부사장
                                </option>
                                <option value="사장" <c:if test="${employeeVO.empPosition eq '사장'}">selected</c:if>>사장
                                </option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label for="inputEmail3" class="form-label">생년월일</label>
                        <div class="input-group">
                            <span class="input-group-text" id="basic-addon1">
                                <i class="ti ti-calendar fs-4"></i>
                            </span>
                            <input type="date" class="form-control" id="empBirth" name="empBirth" aria-label="Username"
                                aria-describedby="basic-addon1" value="${employeeVO.empBirth}">
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        <label class="form-label">우편번호</label>
                        <div class="controls d-flex">
                            <input type="text" id="empPostcode" name="empPostcode" class="form-control"
                                style="width: 93%;" value="${employeeVO.empPostcode}" />
                            <button type="button" id="searchAdd" class="btn btn-primary ms-2"
                    ...
                    <div class="d-flex flex-wrap gap-6 justify-content-end">
                        ...
                        <button type="reset" id="resetBtn" class="btn btn-outline-primary"
                            onclick="location.href='/management/empDetail?empId=${employeeVO.empId}'">
                            이전
                        </button>
                    </div>
                    <sec:csrfInput />
