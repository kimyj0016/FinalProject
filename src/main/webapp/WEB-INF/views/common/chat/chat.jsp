...
//             singleHtml += "</ul>";
   ...
   function send() {
        ...
        $.ajax({
            ...
            success: function(res) {
                  // JSON 형식으로 메시지를 전송
   // 사실 여기서 send했을 때 바로 db에 넣어버리는게 맞는 상황이나 그냥 Controller에 몰아넣기 위해 왔다갔다함.
      ...
      let data;    
        ...
        if (data.type === "userList") {
         ...
         singleRecentMsg();
            ...
            // 일반 메시지 처리
         // 서버에서는 별다른 제어를 하지 않아서 roomNo로 제어를 해주지 않으면 모든 사용자에게
         // 메시지가 무분별하게 뿌려지게 된다. 같은방에 있는 사람에게만 메시지가 전달되도록 함.
...
//            chatBox.scrollTop(chatBox[0].scrollHeight);
   ...
   // 누군가 접속을 종료한다면, session을 다시한번 확인해서 onlineUser, offlineUser를 갱신한다.
         sessionUser(userData);
...
// webSocket을 할때는 handshake라는 요소가 제일 중요하다고 생각함.
// new WebSocket으로 웹소켓이 열릴 때, WebSocketConfig에서 이 연결을 받아서(bean에 경로도 있어야함..)
// ChatHandler.java의 afterConnectionEstablished로 연결된다.
// 그리고 java에서 실행이 끝난 뒤 callback을 해주는게 webSocket.onopen이다.

// 가동 : new webSocket → afterConnectionEstablished(server) → onopen 
// 사용중 : webSocket.send() → handleTextMessage(server) → onmessage 형식으로 이루어짐
