    ...
    <div class="container-fluid">
            ...
            <div class="row mb-4 align-items-center">
	            <div class="col-md-12 d-flex justify-content-end align-items-center">
	                <form action="/partner/facilities/reportList" method="get" class="position-relative me-2" id="searchForm">
					    ...
					    <c:choose>
					        ...
					        <c:otherwise>
					            ...
					            <c:forEach items="${pagingVO.dataList}" var="report" varStatus="status">
					                <tr onclick="location.href='/partner/facilities/reportDetail?inNo=${report.inNo}';" style="cursor: pointer;">
					                    <td><p class="mb-0 fw-normal fs-4">${currentCount - status.index}</p></td> <!-- 1을 빼줌 -->
					                    <td><p class="mb-0 fw-normal fs-4">${report.inGoal}</p></td>
					                    <td class="text-start"><p class="mb-0 fw-normal fs-4" style="margin-left:60px;">${report.inTitle}</p></td>
		    ...
		    <div class="text-end mb-3">
			    <c:choose>
			        <c:when test="${not empty searchWord}">
			            <a href="/partner/facilities/reportList" class="btn btn-outline-primary">목록</a>
			        </c:when>
			        <c:otherwise>
			            <a href="/partner/facilities/reportRegister" class="btn btn-primary" onclick="handleBlindProcessing()">보고서 작성</a>
			        </c:otherwise>
			    </c:choose>
			</div>
